...
2. Run it on a **self-hosted runner on the NYU network**. This repo does exactly that: `.github/workflows/sync-booked-data.yml` runs the fetch scripts on `runs-on: [MEG Workstation]` (a Windows runner, `shell: pwsh`), reading `BOOKED_API_ID` / `BOOKED_API_KEY` from org-level secrets, and commits `equipment.json` / `stats.json` if they changed. The commit to `main` then triggers `deploy-pages.yml` to republish. It runs on a **weekly cron** (`0 6 * * 1`, Mondays 06:00 UTC) plus `workflow_dispatch`, so the published deck self-refreshes once a week with no manual step. (The same runner powers the lab's `neurowaves-lab-documentation/.github/workflows/sync-gcal.yml`.)
...
**Usage stats** (`fetch-stats.mjs` -> `stats.json`, rendered by `<StatCards>` / `<UsageBars>`): pulls `GET /Services/Reservations/` in monthly windows over `STATS_MONTHS` (default 12), dedupes by `referenceNumber`, and writes only **aggregates** (total hours, reservation count, distinct-user tally, instruments used, top-10 by hours, monthly hours), never names or who-booked-what. Reservations need **admin/reservation scope** on the API key; if the key lacks it the fetch 401/403s and fails soft (the workflow step is `continue-on-error`, so the build keeps the committed sample). Run via `npm run data:stats` (or `data:all` for equipment + stats).
...
**Show the refresh command on the slide.** Because there is no live in-deck refresh (a browser fetch would leak the key and Booked is VPN-only; a GitHub-Actions-trigger button is multi-minute async, not live), every slide that renders baked Booked data states its own update command inline so the maintainer sees how to refresh it: `npm run data` on the equipment slide, `npm run data:stats` on the stats/usage slides (`npm run data:all` does both). Keep it on an existing text line, not a new block, to stay within the canvas height budget. When you add a new data slide, add the matching command.
